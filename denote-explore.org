...
1. ~denote-explore-count-notes~: Count the number of notes and attachments. With universal argument, only count attachments.
2. ~denote-explore-count-keywords~: Count the number of used and distinct Denote keywords. 
...
3. ~denote-explore-barchart-timeline~: Visualise notes created by year for the last n years, or all years when omitted.
...
Additionally, the ~denote-explore-identify-duplicate-notes-dired~ command displays files with duplicate identifiers in a Dired buffer. You can directly change filenames in the Dired buffer with ~dired-toggle-read-only~ (=C-x C-q=) or remove individual duplicates with =D= (~dired-do-delete~).
...
The [[https://codeberg.org/treflip/denote-wordcloud][keywords wordcloud package]] by treflip generates an interactive wordcloud in plain text for all your keywords to quickly explore your collection.
...
3. Sequence: Visualise a signature hierarchy with optional linked-note context
...
The ~denote-explore-network-regenerate~ command recreates the previous graph with the same parameters, which is useful when changing the structure of your notes and you like to see the result visualised without having to re-enter the parameters.  Generated JSON, HTML, DOT and GEXF files are snapshots: regenerate them after changing notes, graph options, or the D3 template.  Sequence history saves both its root and context depth.  Older saved Sequence queries containing only a root remain valid and regenerate at depth zero.

Using the universal argument =C-u= before issuing these two commands (re)generates a network excluding attachments.  This text-only policy also applies to new and legacy Sequence queries. The ~denote-excluded-files-regexp~ variable can contain a regular expression of files that are excluded from visualisation.
...
** Sequences Network
The `denote-sequence' package provides an optional extension to `denote' for naming files with a sequence scheme. The package lets the user assign hierarchical relationships between files using signatures, such that the contents of one logically follow or complement those of another. For more information see [[info:denote-sequence][info denote-sequence]]. 

For example a note with signature =1=a= is the child of a note with signature =1= and a note with signature =1=b= is its sibling. The note with signature =1=a=1= is the child of =1=a= and the grandchild of =1=, and so forth.  These immediate-parent hierarchy relationships are separate from actual Denote links in note contents.
...
Users of the `denote-sequence' package can generate a Sequence graph using ~denote-explore-network~. Select the signature of the root node (note =1= in the diagram) as the starting point. When not selecting any signature, all eligible Denote files with a signature are included.  Eligibility follows the normal Denote file and directory exclusions, the text-only choice, and ~denote-explore-network-regex-ignore~; only files with a valid Denote identifier participate.

Each interactive generation prompts for a non-negative context depth.  Its suggested default is the customizable ~denote-explore-network-context-depth~, initially zero.  At depth zero, the graph contains selected sequence notes and only their immediate-parent hierarchy edges; it does not read note bodies to extract links.

At a positive depth, every selected sequence note is a distance-zero source.  Denote-Explorer scans each eligible note source once, expands both incoming and outgoing actual Denote links for the requested number of hops, and includes every directed actual link whose endpoints are loaded.  Thus the exported context is an induced directed graph, including reciprocal, self, and frontier-to-frontier links.  Repeated actual links aggregate into their edge weight without changing reachability; hierarchy edges remain separate even when they have the same endpoints as an actual link.

Sequence metadata reports selected and context-node counts, hierarchy-edge and actual-link counts, and actual-link occurrences.  `degree' and `backlinks' retain their mixed-relationship meanings: degree counts incident relationship records and backlinks sum incoming relationship weights.  `actualDegree' and `actualBacklinks' report only actual Denote-link records and incoming actual-link occurrence weights.  This distinction prevents hierarchy relationships from being presented as document backlinks.

Positive-depth graphs at or above ~denote-explore-network-context-warning-threshold~ (default 1000 loaded nodes) issue one Emacs warning with requested depth and loaded node and edge counts.  The warning never truncates nodes, edges, or depth.
...
Hover over any node to reveal its name and relevant metadata. For neighbourhood and community graphs, when the note is an image or PDF file, a preview appears in the tooltip.  Hover previews remain available independently of node focus.

In every D3 graph type, including Keywords, click a node or press =Enter= or =Space= on its keyboard-focused node to select it.  Selection highlights the node, its incoming and outgoing relationships, and direct neighbours, then pans that node into view at the current zoom.  It never opens a note or fetches more graph data.  Press =Escape= or use the Clear focus button to restore the base styling.  When the operating system requests reduced motion, focus pans and Sequence framing apply without animation.

A Sequence graph frames its sequence members once when its D3 simulation ends, using settled coordinates.  It does not fit early from a cooling alpha value, refit on ticks, or override a prior gesture or selection.

For community and neighbourhood graphs, node size increases with degree. The colours indicate the file type of each node; keyword nodes have equal sizes.  Sequence members retain a halo independently of selection, while context nodes are no larger than the smallest sequence member.  Hierarchy relationships use solid strokes and actual links use dashed strokes.

The label-density slider controls labels without restarting the layout.  At zero (the leftmost setting), every label is hidden, including sequence members and the selected node.  At nonzero settings, visible sequence members and the selected node keep their labels.

Selecting a node shows its details in the sidebar.  Hovering over another node or an edge temporarily previews its details; leaving it restores the selected node's details.  Clearing focus removes the persistent details.

In keyword graphs, the thickness of the edges indicates the number of times two keywords are associated with each other.

The info button shows the type of network and loaded node and relationship counts.  Sequence graphs also report context depth, sequence/context counts, hierarchy/actual-link counts, and actual-link occurrences.  Their density is shown as =N/A (mixed relationships)= rather than comparing different relationship kinds as ordinary links.  Old Sequence data without actual-link metrics explicitly reports those metrics as unavailable.  Other graph types retain their existing network-density statistics.
...
Custom templates made before Sequence context and focus support can ignore the additive Sequence fields, but must be updated to render the Sequence-member hero styling and focus controls.  Regenerate existing HTML after updating its template or graph data; generated HTML embeds a snapshot rather than tracking files live.
...
GraphViz SVG node hyperlinks remain available for note nodes.  D3 node focus, keyboard controls, panning, and Sequence framing are D3-only interactions.
...
The ~denote-explore-network-graphviz-filetype~ variable defines the GraphViz output format. SVG (the default) or PDF provide the best results.  DOT and GEXF exports retain the loaded Sequence topology and edge weights, including expanded actual-link context; they do not add D3 interactions.
...
#+begin_src elisp
  (use-package denote-explore
    ...
    ;; Sequence link-context prompt default and large-context warning threshold
    (denote-explore-network-context-depth 0)
    (denote-explore-network-context-warning-threshold 1000)
