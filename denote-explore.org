...
For community and neighbourhood graphs, node size increases with degree. The colours indicate the file type of each node; keyword nodes have equal sizes.  Sequence members retain a halo and label independently of selection, while context nodes are no larger than the smallest sequence member.  Hierarchy relationships use solid strokes and actual links use dashed strokes.

The label-density slider controls ordinary labels without restarting the layout.  Visible sequence members and the selected node keep their labels at every density setting.
