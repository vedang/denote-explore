...
Generated network files are snapshots.  Regenerate after notes or options change, and regenerate existing D3 HTML after updating its template.  Older custom templates can ignore additive Sequence fields, but need updates for Sequence hero styling and focus controls.  In D3, click or =Enter=/=Space= selects and pans any node, including Keywords, while highlighting its incident relationships and neighbours; it does not open a file.  =Escape= or Clear focus restores base styling, hover previews remain, and reduced-motion preferences suppress camera animation.  Sequence framing occurs once when simulation ends from settled coordinates.  DOT and GEXF retain loaded topology and weights; D3 focus is not added there, while GraphViz SVG hyperlinks remain unchanged.
