...
* Sequence Context and D3 Focus
Sequence generation prompts for a non-negative link-context depth each time; ~denote-explore-network-context-depth~ supplies its default, initially zero.  Depth zero exports selected signature notes and their immediate-parent hierarchy only.  At a positive depth, all selected sequence notes are sources: eligible note files are scanned once, incoming and outgoing actual Denote links expand the context, and every directed actual link between loaded nodes is retained with its weight.  The normal Denote exclusions, text-only choice, and ~denote-explore-network-regex-ignore~ apply.  Large positive-depth graphs warn at ~denote-explore-network-context-warning-threshold~ (1000 loaded nodes by default), but are never truncated.

Sequence graph metadata keeps hierarchy relationships separate from actual links.  `degree' and `backlinks' remain mixed-relationship metrics; `actualDegree' and `actualBacklinks' describe actual Denote links only.  Saved Sequence queries retain root and depth; legacy root-only queries regenerate at depth zero, and =C-u= keeps its text-only meaning.

Generated network files are snapshots.  Regenerate after notes or options change, and regenerate existing D3 HTML after updating its template.  Older custom templates can ignore additive Sequence fields, but need updates for Sequence hero styling and focus controls.  In D3, click or =Enter=/=Space= selects and pans any node, including Keywords, while highlighting its incident relationships and neighbours; it does not open a file.  Selection keeps node details in the sidebar; node and edge hover previews temporarily override them, then restore the selection on mouseout.  =Escape= or Clear focus restores base styling and clears persistent details, and reduced-motion preferences suppress camera animation.  The label-density slider's leftmost setting hides every label; nonzero settings retain sequence-member and selected-node labels.  Sequence framing occurs once when simulation ends from settled coordinates.  DOT and GEXF retain loaded topology and weights; D3 focus is not added there, while GraphViz SVG hyperlinks remain unchanged.

* Change Log
Major version numbers coincide with Denote major versions.

** Unreleased
- Sequence graphs support configurable per-generation actual-link context, typed relationship and actual-link metrics, compatible saved queries, and large-context warnings without truncation.
- D3 node activation focuses and pans instead of opening files, with keyboard, clear-focus, reduced-motion, and settled Sequence-framing support.
- Sequence context topology and weights are retained by DOT and GEXF exports; GraphViz SVG hyperlinks are unchanged.

** Version 4.2
- Minor compatability issues with Emacs 31 (~if-let~)
- Bug fixes and overall improvements to =HTML= template for D#js visualisation
